"""
This script analyzes and visualizes music collection data. It loads music
metadata and audio features, extends the dataset with derived attributes
(e.g., genre, danceability, valence), and generates distribution plots for
various descriptors (e.g., tempo, key, loudness). The results are saved as
TSV files and visual plots for further exploration.
"""
...
DANCEABILITY_DISTRIBUTION_PLOT_PATH = (
    "../results/danceability_distribution.png"
)
...
VALENCE_AROUSAL_DISTRIBUTION_PLOT_PATH = (
    "../results/valence_arousal_distribution.png"
)

# Path to save the instrumental-voice distribution plot
INSTRUMENTAL_VOICE_DISTRIBUTION_PLOT_PATH = (
    "../results/instrumental_voice_distribution.png"
)


def get_music_styles(collection_df, style_metadata_file_path):
    """
    Determine the music style for each track in the collection based on the
    maximum activation value.

    Args:
        collection_df (pd.DataFrame): DataFrame containing the music
            collection data.
        style_metadata_file_path (str): Path to the JSON file containing
            Discogs metadata.
    ...
    music_style_index = collection_df["music_styles_activations"].apply(
        lambda x: x.index(max(x))
    )
...
def get_activation_value(collection_df, column_name, index):
    """
    Extract the activation value at a specific index from a list in a
    DataFrame column.
    ...
    Returns:
        pd.Series: A Series containing the activation values at the specified
            index.
    """
...
def extend_collection_df(collection_df, style_metadata_file_path):
    ...
    Returns:
        pd.DataFrame: Extended DataFrame with additional columns for
            music style, genre, danceability, valence, arousal,
            instrumental, and voice.
    """

    # Explore music styles and genres from the collection
    collection_df["music_style"] = get_music_styles(
        collection_df, style_metadata_file_path
    )
    collection_df["genre"] = get_music_genres(collection_df)

    # Extract danceability, valence, and arousal values
    collection_df["danceability"] = get_activation_value(
        collection_df, "danceability_activations", 0
    )
    collection_df["valence"] = get_activation_value(
        collection_df, "arousal_valence_activations", 0
    )
    collection_df["arousal"] = get_activation_value(
        collection_df, "arousal_valence_activations", 1
    )

    # Extract instrumental and voice values
    instrumental_values = get_activation_value(
        collection_df, "voice_instrumental_activations", 0
    )
    voice_values = get_activation_value(
        collection_df, "voice_instrumental_activations", 1
    )

    # Determine whether a track is instrumental or voice based on its values
    collection_df["instrumental"] = (
        instrumental_values > voice_values
    ).astype(int)
...
def main():
    ...
    collection_df = extend_collection_df(
        collection_df, STYLE_METADATA_FILE_PATH
    )
    ...
    u.save_danceability_distribution_plot(
        collection_df, DANCEABILITY_DISTRIBUTION_PLOT_PATH
    )
    u.save_key_distribution_plot(collection_df, KEY_DISTRIBUTION_PLOT_PATH)
    u.save_loudness_distribution_plot(
        collection_df, LOUDNESS_DISTRIBUTION_PLOT_PATH
    )
    u.save_valence_arousal_distribution_plot(
        collection_df, VALENCE_AROUSAL_DISTRIBUTION_PLOT_PATH
    )
    u.save_instrumental_voice_plot(
        collection_df, INSTRUMENTAL_VOICE_DISTRIBUTION_PLOT_PATH
    )
